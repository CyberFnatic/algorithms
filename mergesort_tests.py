...
def test_merge_sort(how_many):
    ...
    array_init_min, array_init_sec, array_init_ms = time_check(array_init_start, datetime.now())

    # Print how long array init took
    print ("Array's initialization took ", end='')
    time_print(array_init_min, array_init_sec, array_init_ms)   
    

    sort_start = datetime.now()
    merge_sort(array)
    sort_min, sort_sec, sort_ms = time_check(sort_start, datetime.now())

    # Print how long sorting took
    print ("Sorting took ", end='')
    time_print(sort_min, sort_sec, sort_ms) 

def time_print(mins, sec, ms):
    if (mins > 0):
